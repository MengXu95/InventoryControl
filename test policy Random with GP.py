...
class RandomDemand:
    def __init__(self):
        self.list = np.random.uniform(0, 30000, size=(2, epi_len + 3))
...
# Obtain testing demand data from Demand forecast function
num_instances = 10
num_sites = 2
demand_test = []
seed = 666
...
for k in range(num_instances):
    ...
    demand_hist_list = np.array(randomDemand.gen_demand())
    for i in range(len(demand_hist_list[1])): # this is only for consider one site scenario
...
print('\n-----Final comparison with best GP------: ')
print([mean(best_cost_GP), mean(cost_DRL), mean(cost_sS)])
print([np.std(best_cost_GP), np.std(cost_DRL), np.std(cost_sS)])
...
print([mean(best_invlvls_GP), mean(invlvls_DRL), mean(invlvls_sS), mean(best_fill_GP), mean(fill_DRL), mean(fill_sS)])

print('\n-----Final comparison with final gen GP------: ')
print([mean(final_gen_cost_GP), mean(cost_DRL), mean(cost_sS)])
print([np.std(final_gen_cost_GP), np.std(cost_DRL), np.std(cost_sS)])

print('\nDiff between GP and DRL: ')
DRL_diff = [y-x for x,y in zip(final_gen_cost_GP, cost_DRL)]
print(mean(DRL_diff))

print('\nDiff between GP and sS: ')
sS_diff = [y-x for x,y in zip(final_gen_cost_GP, cost_sS)]
print(mean(sS_diff))
...
print('\nFinal inventory level and fill: ')
print([mean(final_gen_invlvls_GP), mean(invlvls_DRL), mean(invlvls_sS), mean(final_gen_fill_GP), mean(fill_DRL), mean(fill_sS)])