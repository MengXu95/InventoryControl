...
def save_individual(randomSeeds, dataSetName,individuals):
    ...
    directory = f'./MTGP_niching_replenish_transship/train/scenario_{dataSetName}/'
...
def save_best_individual_to_txt_for_DemoTest(bestInd): # save individual as txt by mengxu
    # Construct the directory and file path
    directory = f'C:/Users/xum/Desktop/InventoryPolicy/'
    file_path_replenishment_policy = os.path.join(directory, f'replenishment_policy.txt')
    file_path_transshipment_policy = os.path.join(directory, f'transshipment_policy.txt')
    ...
    if len(bestInd) >= 2:
        with open(file_path_transshipment_policy, 'w') as file:
...
def save_TestResults_to_csv(randomSeeds, dataSetName, resultsDf):
    ...
    # Construct the directory and file path
    directory = f'./MTGP_niching_replenish_transship/train/scenario_{dataSetName}/test/'
