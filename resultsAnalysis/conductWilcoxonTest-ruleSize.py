...
for algo, folder in folders.items():
    for scenario in scenarios:
        for run in range(1, runs + 1):
            ...
            gen = len(df[type])
            result = df[type][gen - 1]
            data[scenario].append({'Algorithm': algo, 'Run': run, 'Size': result})

for scenario in data:
    test_fitness_values = [entry['Size'] for entry in data[scenario]]
    worst_value = max(test_fitness_values) if len(test_fitness_values) > 0 else np.nan

    for entry in data[scenario]:
        if np.isnan(entry['Size']):
            entry['Size'] = worst_value
...
def perform_wilcoxon_test(scenario_data, algorithms):
    ...
    for i, algo1 in enumerate(algorithms):
        for algo2 in algorithms[i + 1:]:
            data_algo1 = scenario_data[scenario_data['Algorithm'] == algo1]['Size']
            data_algo2 = scenario_data[scenario_data['Algorithm'] == algo2]['Size']
...
def calculate_mean_std(data, algorithms):
    ...
    for scenario in scenarios:
        ...
        for algo in algorithms:
            algo_data = scenario_data[scenario_data['Algorithm'] == algo]['Size']
...
mean_std_df.to_csv(os.path.join(workdir, "mean_std_rule_size_results_" + type + "_" + scenarios_type + ".csv"), index=False)

mean_std_df_easy_for_table = pd.DataFrame(mean_std_results_easy_for_table, columns=['Scenario', 'Algorithm', 'Mean', 'Best'])
mean_std_df_easy_for_table.to_csv(os.path.join(workdir, "mean_std_rule_size_results_easy_for_table_" + type + "_" + scenarios_type + ".csv"), index=False)
