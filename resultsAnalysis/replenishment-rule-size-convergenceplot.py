...
for algo, folder in folders.items():
    for scenario in scenarios:
        for run in range(1, runs + 1):
            ...
            gen = len(df['RepRuleSize'])
            for i in range(gen):
                result = df['RepRuleSize'][i]
                data[scenario].append({'Algorithm': algo, 'Run': run, 'Gen': i, 'RepRuleSize': result})

for scenario in data:
    test_fitness_values = [entry['RepRuleSize'] for entry in data[scenario]]
    worst_value = max(test_fitness_values) if len(test_fitness_values) > 0 else np.nan

    for entry in data[scenario]:
        if np.isnan(entry['RepRuleSize']):
            entry['RepRuleSize'] = worst_value
...
for i, scenario in enumerate(scenarios):
    ...
    mean_test_fitness = scenario_data.groupby(['Gen', 'Algorithm'])['RepRuleSize'].mean().reset_index()

    print("Replenishment rule size across generations: " + str(mean_test_fitness))
...
fig.write_image(workdir + "RepRuleSize_convergence_curves.png")
