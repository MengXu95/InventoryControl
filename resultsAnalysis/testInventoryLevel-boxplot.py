    ...
    for scenario in scenarios:
        for run in range(1, runs + 1):
            if algo == 'sSPolicy':
                ...
                result = df['InveLevel'].iloc[0]
                data[scenario].append({'Algorithm': algo, 'Run': run, 'InveLevel': result})
            else:
                file_path = os.path.join(folder, f'scenario_{scenario}/test/{run}_{scenario}_testResults.csv')
                df = pd.read_csv(file_path)
                gen = len(df['InveLevel'])
                result = df['InveLevel'][gen - 1]
                data[scenario].append({'Algorithm': algo, 'Run': run, 'InveLevel': result})


for scenario in data:
    test_fitness_values = [entry['InveLevel'] for entry in data[scenario]]
    worst_value = max(test_fitness_values) if len(test_fitness_values) > 0 else np.nan

    for entry in data[scenario]:
        if np.isnan(entry['InveLevel']):
            entry['InveLevel'] = worst_value
...
for i, scenario in enumerate(scenarios):
    ...
    y_min = scenario_data['InveLevel'].min()
    y_max = scenario_data['InveLevel'].max()

    for algo in algorithms:
        algo_data = scenario_data[scenario_data['Algorithm'] == algo]
        fig.add_trace(
            go.Box(
                y=algo_data['InveLevel'],
                name=algo,
...
fig.write_image(workdir + "test_InveLevel_comparison_" + scenarios_type + ".pdf")
