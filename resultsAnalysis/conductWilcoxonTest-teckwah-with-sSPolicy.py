...
scenarios = ["teckwah_training"]
instances = ["industry_1", "industry_2", "industry_3", "industry_4", "industry_5",
             "industry_6", "industry_7", "industry_8", "industry_9", "industry_10",
             "industry_11", "industry_12", "industry_13", "industry_14", "industry_15",
             "industry_16", "industry_17", "industry_18", "industry_19", "industry_20"]
scenarios_type = 'teckwah'
...
data = {instance: [] for instance in instances}
...
for algo, folder in folders.items():
    for scenario in scenarios:
        for run in range(1, runs + 1):
            if algo == 'sSPolicy':
                file_path = os.path.join(folder, f'scenario_{scenario}/{run}_{scenario}_sSPolicy_test_results_each_instance.csv')
                df = pd.read_csv(file_path)
                for i in range(len(instances)):
                    instance = instances[i]
                    result = df['TestFitness'].iloc[i]
                    data[instance].append({'Algorithm': algo, 'Run': run, 'TestFitness': result})
            else:
                file_path = os.path.join(folder, f'scenario_{scenario}/test/{run}_{scenario}_testResults_final_gen_each_instance.csv')
                df = pd.read_csv(file_path)
                for i in range(len(instances)):
                    instance = instances[i]
                    result = df['TestFitness'].iloc[i]
...
def calculate_mean_std(data, algorithms):
    ...
    for instance in instances:
        scenario_data = pd.DataFrame(data[instance])
...
for instance in instances:
    ...
    wilcoxon_results[instance] = results
