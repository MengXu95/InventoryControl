...
import threading
from Utils.inventory_core import RentalRetailer as Retailer, build_demand_model
...
class InvOptEnv:
    ...
    def step_value(self, action_modified):  # modified by mengxu to make it not only suitable for 2 sites
        if len(self.retailers) == 2:
            ...
            for each_rental_decision in rental_decisions:
                # print("each_rental_decision: ", each_rental_decision)
                # if each_rental_decision > len(self.rental_choice) or each_rental_decision < 0:
                #     print("each_rental_decision: " + each_rental_decision)
    ...
    def PSO_run(self, individual): # add by xumeng 2024.8.1
        # run simulation
        state = self.reset()
        current_ep_reward = 0
        ...
        for i in range(1, max_ep_len + 1):
            ...
            decisions = individual[i]
            replenishment_quantities = decisions[0]
            rental_decisions = decisions[-1]
            ...
            for j in range(len(replenishment_state)):
                each_replenishment_state = replenishment_state[j]
                replenishment_quantity = round(replenishment_quantities[j], 2)
    ...
    def PSO_run_test(self, individual, states=None, actions=None, rewards=None): # add by xumeng 2024.8.1
        # run simulation
        state = self.reset()
        current_ep_reward = 0
