...
def find_stock_on_hand_column(df):
    """
    Dynamically find the stock on hand column regardless of date
    """
    # Look for columns that match the pattern for stock on hand
    stock_patterns = [
        r'.*stock\s+on\s+hand.*',
...
def find_po_balance_column(df):
    """
    Dynamically find the PO balance column regardless of date
    """
    # Look for columns that match the pattern for PO balance
    po_patterns = [
        r'.*po\s+bal.*',
        r'.*system\s+po.*',
...
def find_input_file():
    """
    Find inventory Excel file in current directory
    """
    # Common patterns for inventory files
    patterns = [
        'inventory*.xlsx',
        '*inventory*.xlsx',
        'stock*.xlsx',
...
def generate_output_filename(input_file=None):
    """
    ...
    if input_file:
        ...
        return f"{base_name}_ordering_recommendations_{timestamp}.xlsx"
    else:
        return f"ordering_recommendations_{timestamp}.xlsx"
...
def calculate_safety_stock(demands, leadtime_weeks):
    ...
    # Remove zero/negative values and convert to integers
    clean_demands = [safe_int_convert(d) for d in demands if safe_int_convert(d) > 0]
...
def inventory_ordering_policy(row, month_columns, stock_col, po_col):
    """
    Implement inventory ordering policy for a single item
    Returns: (monthly_orders, monthly_inventory_levels) - both as integer arrays
    """
    # Extract basic information
    leadtime_weeks = parse_leadtime(row.get('Revised Leadtime'))
    stock_on_hand = safe_int_convert(row.get(stock_col, 0))
    po_balance = safe_int_convert(row.get(po_col, 0))

    # Extract monthly demands and convert to integers
    monthly_demands = []
    for month in month_columns:
        demand = safe_int_convert(row.get(month, 0))
    ...
    for month_idx in range(num_months):
        ...
        if projected_inventory_after_demand < 0:
            times_out_of_stock += 1
...
def process_inventory_file(input_file_path, output_file_path):
    ...
    # Dynamically find stock and PO columns
    stock_col = find_stock_on_hand_column(df)
    po_col = find_po_balance_column(df)

    if stock_col is None:
        print("Warning: Could not find stock on hand column. Using default value of 0.")
        stock_col = 'dummy_stock_col'
        df[stock_col] = 0

    if po_col is None:
        print("Warning: Could not find PO balance column. Using default value of 0.")
        po_col = 'dummy_po_col'
        df[po_col] = 0

    print(f"Using stock column: {stock_col}")
    print(f"Using PO balance column: {po_col}")
    ...
    for month in month_columns:
        if month in df.columns:
            output_df[f'Demand_{month}'] = df[month].apply(safe_int_convert)

    # Add starting inventory information for reference (as integers)
    if stock_col in df.columns:
        output_df['Starting_Stock_on_Hand'] = df[stock_col].apply(safe_int_convert)
    if po_col in df.columns:
        output_df['Starting_PO_Balance'] = df[po_col].apply(safe_int_convert)

    # Calculate orders for each row
    for idx, row in df.iterrows():
        monthly_orders, monthly_inventory_levels = inventory_ordering_policy(row, month_columns, stock_col, po_col)
...
if __name__ == "__main__":
    ...
    if input_file is None:
        print("Error: No Excel inventory file found in current directory.")
        print("Please ensure your inventory Excel file is in the current directory.")
        print("Common file patterns searched: inventory*.xlsx, *inventory*.xlsx, stock*.xlsx, *stock*.xlsx")
        exit(1)
    ...
    try:
        ...
        print(f"\n✅ SUCCESS: Ordering recommendations saved to: {output_file}")
