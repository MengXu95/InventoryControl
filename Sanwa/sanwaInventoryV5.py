...
def inventory_ordering_policy(row, month_columns):
    ...
    stock_on_hand = safe_int_convert(row.get('Sanwa stock on hand 27/11/2024', 0))
    po_balance = safe_int_convert(row.get('Sanwa system PO bal as at ', 0))
    ...
    # print("Sanwa item", row.get('sanwa item'))
    ...
    for month_idx in range(num_months):
        ...
        for j in range(look_ahead_months):
            if month_idx + j < num_months:
                ...
                # todo: Meng Xu further consider any deliveries from previous orders and will arrive within the look ahead months
                if j > 0:
                    future_demand -= monthly_deliveries[month_idx + j]

        if future_demand < 0:
            # print("Error here, future demand should not be < 0! It is: ", future_demand)
            future_demand = 0
        ...
        if projected_inventory_after_demand < 0:
            ...
            # print("Error here, projected_inventory_after_demand should not be < 0! It is: ", projected_inventory_after_demand)
        ...
        if projected_inventory_after_demand < reorder_point:
            ...
            if moq > 0 and order_qty > 0:
                ...
                # this depends, whether it must be like order the multiple of MOQ
                # if order_qty > moq:
            ...
            delivery_month = month_idx + leadtime_months # original
            # delivery_month = month_idx + leadtime_months - 1
    ...
    # print("Times_out_of_stock = ", times_out_of_stock)
...
def process_inventory_file(input_file_path, output_file_path):
    ...
    if 'Sanwa stock on hand 27/11/2024' in df.columns:
        output_df['Starting_Stock_on_Hand'] = df['Sanwa stock on hand 27/11/2024'].apply(safe_int_convert)
    if 'Sanwa system PO bal as at ' in df.columns:
        output_df['Starting_PO_Balance'] = df['Sanwa system PO bal as at '].apply(safe_int_convert)
    ...
    with pd.ExcelWriter(output_file_path, engine='openpyxl') as writer:
        ...
        # Light blue for the first order column
...
if __name__ == "__main__":
    ...
    output_file = "C:/Users/I3Nexus/Desktop/ordering_recommendations_V5.xlsx"
