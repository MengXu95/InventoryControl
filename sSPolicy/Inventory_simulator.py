...
from sSPolicy.sS_policy import sS_action
import threading
import time
...
class InvOptEnv:
    ...
    def run_sSPolicy(self, sSPolicy, GP_states=None, GP_actions=None, GP_rewards=None): # add by xumeng 2024.8.1
        # run simulation
        state = self.reset()
        current_ep_reward = 0
        ...
        for _ in range(1, max_ep_len + 1):
            ...
            start = time.time()
            for _ in transshipment_state:
                transshipment_quantity = 0
                action_modified.append(transshipment_quantity)
            for index, each_replenishment_state in enumerate(replenishment_state):
                replenishment_quantity = round(sS_action(each_replenishment_state, sSPolicy[index]),2)
