...
class ResinInventoryManager:
    ...
    def aggregate_by_resin(self, df):
        ...
        for col in demand_cols:
            df[f'{col}_kg'] = df.apply(
                lambda row: self.safe_numeric_convert(row[col])/float(self.safe_numeric_convert(row['CAV'])) * row['Weight_kg'],
                axis=1
            )
            # df[f'{col}_Units'] = df.apply(
            #     lambda row: self.safe_numeric_convert(row[col]),
            #     axis=1
            # )
        ...
        # 1. Convert inventory to numeric first
        df['Available_Finish_Good_Unit'] = df['Current finish good inventory (units)'].apply(
            self.safe_numeric_convert
        )

        # 2. Calculate weight using the already converted column and handle potential ZeroDivision
        def calculate_kg(row):
            units = row['Available_Finish_Good_Unit']
            cav = self.safe_numeric_convert(row['CAV'])
            weight = row['Weight_kg']

            # Ensure cav is a float and not zero to avoid crashes
            cav_val = float(cav) if cav and float(cav) != 0 else 1.0

            return (units / cav_val) * weight

        df['Available_Finish_Good_kg'] = df.apply(calculate_kg, axis=1)

        # df['Available_Finish_Good_Unit'] = df['Current finish good inventory (units)'].apply(
        #     lambda x: self.safe_numeric_convert(x)
        # )
        #
        # df['Available_Finish_Good_kg'] = df.apply(
        #         lambda row: self.safe_numeric_convert(row['Current finish good inventory (units)'])/float(self.safe_numeric_convert(row['CAV'])) * row['Weight_kg'],
        #         axis=1
        #     )
        ...
        agg_dict = {
            ...
            'Open_Order_kg': 'sum',
            'Available_Finish_Good_Unit': 'sum',
            'Available_Finish_Good_kg': 'sum',
        }
    ...
    def calculate_ordering_recommendation(self, row, demand_cols, leadtime_weeks=4,
                                          safety_factor=1.2, consider_finish_good=True):
        """
        ...
        # todo: add by mengxu, the above one do not consider finish good;
        #  if consider finish good
        if consider_finish_good:
            approximate_inventory_by_current_finish_good = self.safe_numeric_convert(row.get('Available_Finish_Good_kg', 0))
            current_inventory = current_inventory + approximate_inventory_by_current_finish_good
    ...
    def process_inventory(self, input_file_path, output_file_path=None,
        ...
        for idx, row in aggregated.iterrows():
            calc = self.calculate_ordering_recommendation(
                row, demand_cols, leadtime_weeks, safety_factor, consider_finish_good=True
            )
            ...
            rec = {
                ...
                'Available_Finish_Good_Unit': round(row['Available_Finish_Good_Unit'], 0),
                'Available_Finish_Good_kg': round(row['Available_Finish_Good_kg'], 3),
...
def main():
    ...
    output_file = "resin_ordering_recommendations_new2.csv"
