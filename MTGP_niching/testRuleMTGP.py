...
def main_scenario_sensitivity_analysis(scenario_dataset_name, dataset_name, run):
    # get parameters for the given dataset/scenario
    # scenario_dataset_name = 'sN2h_1_10b3'
    scenarioDesign = ScenarioDesign(scenario_dataset_name)
    parameters = scenarioDesign.get_parameter()
    ...
    PC_diversity = []
    final_gen_each_instance = []
    for idx in range(len(all_gen_individuals)-1,len(all_gen_individuals)):
        print("Generation: " + str(idx))
        individual = all_gen_individuals.get(idx)
        fitness = 0
        invlvls_GP = []
    ...
    if dataset_name == "teckwah_test":
        ...
        # mtsave.save_TestResults_to_csv(run,scenario_dataset_name,df)
        mtsave.save_TestResults_secnario_sensitivity_analysis_to_csv(run, scenario_dataset_name, dataset_name, df)
