...
3. **Cost Prediction Policy Module** (`cost_predict_policy.py`) - Predicts costs based on request for quote (RFQ) and time until delivery (TUD)
...
- [Module 3: Cost Prediction Policy](#module-3-cost-prediction-policy)
...
# Run cost prediction policy demo
python cost_predict_policy.py
```
...
**Cost Prediction Policy:**
```python
from cost_predict_policy import predict_cost

# Predict cost based on RFQ and TUD
cost = predict_cost(
    rfq=100.0,
    tud=5.0,
...
## Module 3: Cost Prediction Policy

### Purpose

Predicts costs based on Request for Quote (RFQ) and Time Until Delivery (TUD) using genetic programming-based policy expressions.
...
```python
predict_cost(rfq, tud, policy_expression=None)
```
...
**Returns:**
- `float`: Predicted cost, rounded to 2 decimal places
...
# Simple case: cost is RFQ divided by TUD
rfq_value = 100.0
tud_value = 5.0

# Use default policy
predicted_cost = predict_cost(rfq_value, tud_value)
print(f"Predicted cost: ${predicted_cost:.2f}")
# Output: Predicted cost: $20.00

# Custom policy: weighted combination
custom_policy = "add(protected_div('RFQ', 'TUD'), multiply('RFQ', 0.1))"
custom_cost = predict_cost(rfq_value, tud_value, policy_expression=custom_policy)
print(f"Custom predicted cost: ${custom_cost:.2f}")
```
...
print(f"{'RFQ':<10} {'TUD':<10} {'Predicted cost':<20}")
print("-" * 40)
for rfq, tud in quotes:
    cost = predict_cost(rfq, tud, policy_expression=policy)
    print(f"{rfq:<10.1f} {tud:<10.1f} ${cost:<19.2f}")

# Output:
# RFQ        TUD        Predicted cost     
# ----------------------------------------
# 150.0      10.0       $15.00              
# 80.0       4.0        $20.00              
# 200.0      8.0        $25.00              
...
### cost Prediction Policy Variables
...
### cost Prediction Policy Examples

**Example 5: Simple Linear cost Model**

cost inversely proportional to delivery time:

```python
policy = "protected_div('RFQ', 'TUD')"
# If RFQ = 100 and TUD = 5, cost = 20
```
...
**Example 7: Sigmoid-Scaled cost**

Use logistic function to scale costs smoothly:

```python
policy = "multiply('RFQ', lf(protected_div('RFQ', 'TUD')))"
# Applies sigmoid transformation for smooth cost curves
```
...
# Debug cost prediction policy
simple_policy = "protected_div('RFQ', 'TUD')"
result = predict_cost(100.0, 5.0, simple_policy)
...
cost = predict_cost(100.0, 5.0, "protected_div('RFQ', 'TUD')")
...
# Higher costs for faster delivery requirements
...
# Ensure cost has a minimum floor
...
# cost prediction with evolved policy
cost_policy = "multiply(lf(protected_div('RFQ', 'TUD')), add('RFQ', square('TUD')))"
cost = predict_cost(rfq, tud, policy_expression=cost_policy)
```
...
**Process Multiple cost Quotes:**
```python
quotes = [(100, 5), (150, 10), (80, 4), (200, 8)]
policy = "protected_div('RFQ', 'TUD')"

costs = [predict_cost(rfq, tud, policy) for rfq, tud in quotes]
print("Predicted costs:", costs)
```
...
### cost Prediction in Real-Time Systems

```python
def dynamic_pricing_engine(rfq_stream, tud_stream, policy):
    """
    Real-time cost prediction for incoming quotes
    """
    costs = []
    for rfq, tud in zip(rfq_stream, tud_stream):
        cost = predict_cost(rfq, tud, policy)
        costs.append(cost)
        # Apply business rules
        if cost < minimum_acceptable_cost:
            cost = minimum_acceptable_cost
        yield cost
```
...
```python
# First, predict the cost
predicted_cost = predict_cost(rfq, tud, cost_policy)

# Then, use cost to influence replenishment decision
...
```python
# Instead of loop
costs = [predict_cost(rfq, tud, policy) for rfq, tud in data]
