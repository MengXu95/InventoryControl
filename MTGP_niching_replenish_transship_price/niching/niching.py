import MTGP_niching_replenish_transship_price.niching.PhenoCharacterisation as PhenoCharacterisation
import numpy as np
import MTGP_niching_replenish_transship_price.replenishment as sequencing
import \
    MTGP_niching_replenish_transship_price.niching.ReplenishmentPhenoCharacterisation as ReplenishmentPhenoCharacterisation
import \
    MTGP_niching_replenish_transship_price.niching.TransshipmentPhenoCharacterisation as TransshipmentPhenoCharacterisation
import MTGP_niching_replenish_transship_price.niching.RFQPredictPhenoCharacterisation as RFQPredictPhenoCharacterisation
from MTGP_niching_replenish_transship_price.niching.Inventory_simulator_replenish_transship_price_niching import *
    ...
    def getDecisionSituations(self):
        ...
        if len(self.decision_points) == 1:  # only consider replenishment
            ...
            if len(transshipment_decision_points) < self.num_decision_points:
                print("Error in get enough number of self.transshipment_decision_points)")

            np.random.shuffle(transshipment_decision_points)
            subset_transshipment_decision_points = transshipment_decision_points[:self.num_decision_points]
            decisionSituations.append(subset_transshipment_decision_points)

        elif len(self.decision_points) == 3:  # consider replenishment, transshipment, and RFQ_predict
            replenishment_decision_points = self.decision_points[0]
            if len(replenishment_decision_points) < self.num_decision_points:
                print("Error in get enough number of self.replenishment_decision_points)")
...
class niching_clear:
    ...
    def initial_phenoCharacterisation(self, parameters, individual):
        ...
        if len(individual) == 1:  # for the instances that only consider the replenishment rule
        ...
        elif len(individual) == 2:  # for the instances that consider both replenishment and transshipment
            replenishmentDecisionSituation = ReplenishmentDecisionSituation.ReplenishmentDecisionSituation(
                self.decisionSituations[0])
            replenishmentPhenoCharacterisation = ReplenishmentPhenoCharacterisation.ReplenishmentPhenoCharacterisation(
                individual[0], replenishmentDecisionSituation)
            ...
            transshipmentDecisionSituation = TransshipmentDecisionSituation.TransshipmentDecisionSituation(
                self.decisionSituations[1])
            transshipmentPhenoCharacterisation = TransshipmentPhenoCharacterisation.TransshipmentPhenoCharacterisation(
                individual[1], transshipmentDecisionSituation)
            self.phenotypic_characristics.append(transshipmentPhenoCharacterisation)

        elif len(individual) == 3:  # for the instances that consider replenishment, transshipment, and RFQ prediction
            replenishmentDecisionSituation = ReplenishmentDecisionSituation.ReplenishmentDecisionSituation(
                self.decisionSituations[0])
            replenishmentPhenoCharacterisation = ReplenishmentPhenoCharacterisation.ReplenishmentPhenoCharacterisation(
                individual[0], replenishmentDecisionSituation)
            ...
            RFQ_predictDecisionSituation = RFQPredictDecisionSituation.RFQPredictDecisionSituation(
                self.decisionSituations[2])
            RFQ_predictPhenoCharacterisation = RFQPredictPhenoCharacterisation.RFQPredictPhenoCharacterisation(
                individual[2], RFQ_predictDecisionSituation, parameters['demand_level'])
    ...
    def clearPopulation(self, toolbox, population, Clear=True):
        ...
        elif len(population[0]) == 3:  # consider replenishment, transshipment, and RFQ predict
            clearedInds = 0
            phenotypic_characristics_pop = []
            sorted_pop = self.sortPopulation(toolbox, population)
            isCleared_pop = []
    ...
    def sortPopulation(self, toolbox, population):
        ...
        # FOR CHECK
        # pop_fit = [np.sum(ind.fitness.values) for ind in populationCopy]
        # print(pop_fit)
