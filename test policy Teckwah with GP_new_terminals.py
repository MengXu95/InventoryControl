...
import MTGP_new_terminals.LoadIndividual as mtload
...
# Get next state and reward given feasible action that has no transshipment
def any_step_new_terminals(state, action, current_period, demand, trans=0):  # Action is a 2-vector of order quantities
    if trans != 0:
        if trans > 0 and state[0] < trans:
            trans = 0
        elif trans < 0 and state[1] < -trans:
    ...
    # print(pipeline2)
    # next_state = np.array(new_state + forecast1 + forecast2 + pipeline1 + pipeline2)
    next_state = np.array([new_state[0]] + forecast1 + pipeline1 + [holding[0]] + [lost_sales[0]] + [fixed_order[0]])
    # print(next_state)
    action_taken = [trans, action[0], action[1]]
    return next_state, action_taken, reward
...
for idx in range(len(dict_best_individuals)):
    ...
    for demand in demand_test:
        ...
        state = [ini_inv[0], demand[0, 0], demand[0, 1], 0, holding[0], lost_sales[0], fixed_order[0]]  # Initial state
        ...
        while current_period <= epi_len:
            ...
            state, action, reward = any_step_new_terminals(state, action[1:], current_period, demand, trans = trans) # Remove first element (transshipment) of action
