...
capacity = [200000,60] # Inventory capacities
fixed_order = [10000,10] # Fixed order costs per order
...
print([mean(cost_DRL), mean(cost_sS)])
print([stdev(cost_DRL), stdev(cost_sS)])

sS_diff = [y-x for x,y in zip(cost_DRL, cost_sS)]
print(mean(sS_diff))

print([mean(invlvls_DRL), mean(invlvls_sS), mean(fill_DRL), mean(fill_sS)])